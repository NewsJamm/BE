package com.NewsJam.NewsJam.global.security.jwt.service;
...
public class JwtService {
    ...
    private static final String PROVIDER_ID_CLAIM = "provider_id";
    private static final String PROVIDER_TYPE_CLAIM = "provider_type";
     ...
     * AccessToken 생성
     */
    public String createAccessToken(AuthProvider authProvider, String providerId) {
        SecretKey key = Keys.hmacShaKeyFor(secretKey.getBytes(StandardCharsets.UTF_8));
        return Jwts.builder()
                .subject(ACCESS_TOKEN_SUBJECT)
                .issuedAt(new Date())
                .expiration(new Date((new Date()).getTime() + accessTokenExpirationPeriod))
                .claim(PROVIDER_TYPE_CLAIM, authProvider.toString())
     ...
     */
    public Optional<String> extractProviderId(String accessToken) {
        ...
        return Optional.ofNullable(String.valueOf(claims.get(PROVIDER_ID_CLAIM)));
    }

    /**
     * AccessToken에서 ProviderType 추출
     */
    public Optional<AuthProvider> extractProviderType(String accessToken) {
        ...
        String value = String.valueOf(claims.get(PROVIDER_TYPE_CLAIM));
    ...
    public void updateRefreshToken(AuthProvider authProvider, String providerId, String refreshToken) {
        memberRepository.findByAuthProviderAndProviderId(authProvider, providerId)
                .ifPresentOrElse(
                        member -> member.changeRefreshToken(refreshToken),
                        () -> new UsernameNotFoundException("일치하는 회원이 없습니다.")
                );
